...
"""This pipeline is designed to align, run fastQC, and generate counts for human, second-stranded, paired-end, 50 bp RNA-seq libraries.
    This is the pipeline used to prcoess all young and aged HSCe RNAseq data.
    One input configuration file 'RNASampleID.txt' is required. It must be a 3 column tab seperated file containing the following:
...
def run_rnaseq(x):
    cmd1 = 'cutadapt -a AGATCGGAAGAGC -A AGATCGGAAGAGC -m 47 --length 48 -o {name}_R1_trim.fastq.gz  -p {name}_R2_trim.fastq.gz  {R1} {R2}'

    cmd1 = cmd1.format(name=x[0], R1=x[1], R2=x[2])
    print '=' * 30
    print cmd1
    os.system(cmd1)
    cmd2 = '/root/FastQC/fastqc  --noextract -f fastq -t 5 {name_R1_trim} ;fastqc  --noextract -f fastq -t 5 {name_R2_trim}'
    ...
    print cmd2
    os.system(cmd2)
    cmd3 = 'STAR --runThreadN 25  --genomeDir /PathToIndices/ --readFilesIn {name_R1_trim} {name_R2_trim} --outFileNamePrefix {name}_ --outFilterType BySJout --outFilterMultimapNmax 20 --alignSJoverhangMin 8 --alignSJDBoverhangMin 1 --outFilterMismatchNmax 999 --alignIntronMin 20 --alignIntronMax 1000000 --alignMatesGapMax 1000000  --readFilesCommand gunzip -c  --outSAMtype BAM SortedByCoordinate --outWigType bedGraph read1_5p --outWigStrand Stranded --outWigNorm RPM'
    ...
    # Prior to calling counts, create gtf file 'gencode.v19_woRNA.gtf' that does not contain rRNA, mitoRNA, or tRNA
    cmd5 = 'java -jar /root/QoRTs.jar QC  --nameSorted --maxReadLength 52 --generatePlots --stranded --stranded_fr_secondstrand {cmd4_out}  /path/gencode.v19_wo_rRNA.gtf {counts_dir}/'
